...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navLinks = [
    ...
    { href: '/tools/modul-ajar', label: 'Modul Ajar & RPP', active: pathname.startsWith('/tools/modul-ajar') },
    { href: '/#katalog', label: 'Semua Tools', active: false },
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-200/90 bg-white/95 backdrop-blur-md shadow-2xs print:hidden">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white px-4 py-3 space-y-3 animate-fade-in shadow-xl">
          <div className="grid grid-cols-1 gap-1.5">
            {navLinks.map((link) => (
              <Link
                ...
                className={`min-h-[44px] px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold flex items-center justify-between transition-colors ${
                  link.active
              ...
              >
                ...
                {link.active && <span className="w-1.5 h-1.5 rounded-full bg-blue-600" />}
              </Link>
            ))}
          </div>
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            ...
            <button
              ...
              className="text-amber-600 font-bold hover:underline flex items-center gap-1 cursor-pointer py-1 px-2 rounded-lg hover:bg-amber-50"
            >
